...
| Evaluator-stress trigger-policy analysis | best trigger policy mean balanced accuracy 0.927934 | autonomous 0.924857; always-on gate 0.926426 | best trigger delta +0.003077; 2 wins, 0 losses, 23 ties; 5 trigger events | A class-imbalance trigger preserves the imbalance benefit while avoiding the clean-task overreach losses of the always-on gate. |
...
We therefore analyze trigger policies over the same 25 paired selections. The always-on evaluator-stress policy has 2 wins, 2 losses, and 21 ties versus autonomous selection. A simple class-imbalance trigger, activated only when the validation majority-class baseline is at least 0.94, triggers on 5 dataset-split cases, obtains 2 wins, 0 losses, and 23 ties, and raises mean balanced accuracy to 0.927934 (delta +0.003077 versus autonomous). This is post-hoc design evidence, not a prospective causal result. Its methodological value is that it converts the mixed pilot into a concrete participation-mode rule: human/evaluator intervention should be gated by evaluator-risk signals rather than left permanently on.

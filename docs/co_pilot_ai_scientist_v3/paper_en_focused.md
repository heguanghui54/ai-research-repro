...
| Live four-condition TFR replay | raw model judge labels 2/2 cases positive | strict preregistered rule relabels 2/2 mixed | 2 cases; 4 live same-model calls; strict positives 0 | Shows why TFR needs deterministic rule checking: model optimism is corrected when the short-term penalty condition is not met. |
| Cross-model TFR replay judge | Claude selects six-gate hybrid as frontier winner in 2/2 cases | strict label remains mixed in 2/2; Gemini malformed JSON in one attempted judge | 2 successful Claude judges | Reduces same-model bias and identifies concrete failure modes: gate actions often improve frontier framing without propagating into datasets, metrics, or mechanisms. |
...
| TFR can test whether past reviews would have moved automated science toward later frontiers. | Supported as an operationalized replay protocol; candidate mining screens 120 comments, a small OpenAlex validation gives delayed candidates a +0.064 lexical frontier advantage over controls, and two live four-condition replays have been executed. | Current validated probes find 0 strict delayed-value cases; both live replays are mixed after preregistered rule repair, so the long-horizon hypothesis remains unproven. |
...
We then execute two preregistered four-condition replays. The first is
`paper_105_review_1`, the E2ENet medical-segmentation case; the second is
`paper_132_review_2`, an averaged-SGD under NTK case. Each replay generates
paper-only, raw-review-guided, six-gate-hybrid-guided, and
shuffled-review-control artifacts. In both cases, a Monica-routed `gpt-4o-mini`
...
met. Claude cross-model review keeps both cases mixed while selecting the
six-gate artifact as the frontier winner in both. The diagnoses are useful. In
...
as feature learning and kernel approximation error. These are not
delayed-value successes. They are executed evidence that TFR can catch
model-judge optimism and expose where gate routing fails to affect experiment
design.
...
The current evidence is a pilot package. It does not include independent human expert review of the final IGRE paper or of the paired regenerated artifacts. The live co-pilot trace is a single-author derived metadata corpus, not a population-level dataset of many scientists using the system. OpenReview is offline asynchronous review data, not real-time human intervention inside an AI Scientist-v2 run. The matched-budget FML evidence is underpowered and currently negative or mixed for benchmark performance. The equal-context OpenReview ablation reduces, but does not eliminate, concerns about context confounds because it still relies on model-routed scoring and regenerated mini-artifacts rather than blind expert review. The delayed-value candidate-mining screen and OpenAlex validation prioritize future replay cases, and the two live TFR replays show model-optimism correction rather than positive delayed-value evidence; they do not replace benchmark reruns, human expert judgement, or larger replay samples. The high-tail hypothesis is now statistically operationalized as a power-analysis protocol, but no high-tail or delayed-value positive case has been demonstrated in the current evidence package.

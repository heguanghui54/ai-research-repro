...
| Evaluator-stress trigger-policy analysis | held-out best trigger mean balanced accuracy 0.924320 | held-out autonomous 0.920681; held-out always-on gate 0.923263 | held-out trigger delta +0.003639; 2 wins, 0 losses, 23 ties; 5 trigger events | A class-imbalance trigger selected on the first split set remains best on held-out split seeds, preserving imbalance benefit while avoiding clean-task overreach. |
...
We therefore analyze trigger policies over the same 25 paired selections, then repeat the analysis on 25 held-out split-seed selections. On the first split set, the always-on evaluator-stress policy has 2 wins, 2 losses, and 21 ties versus autonomous selection. A simple class-imbalance trigger, activated only when the validation majority-class baseline is at least 0.94, triggers on 5 dataset-split cases, obtains 2 wins, 0 losses, and 23 ties, and raises mean balanced accuracy to 0.927934 (delta +0.003077 versus autonomous). On held-out split seeds, the same trigger remains the best parsimonious policy: it obtains mean balanced accuracy 0.924320, delta +0.003639 versus autonomous, 2 wins, 0 losses, and 23 ties, while the always-on gate still has 2 losses. This is split-heldout design evidence within one open-data task family, not a prospective causal result across new task families. Its methodological value is that it converts the mixed pilot into a concrete participation-mode rule: human/evaluator intervention should be gated by evaluator-risk signals rather than left permanently on.

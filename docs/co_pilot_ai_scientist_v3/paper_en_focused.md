...
| Prospective matched packages | 3 co-pilot or human-selected wins, including SSH Max-Cut mean normalized score 0.984419 and open-data evaluator-stress mean 0.934008 | 3 autonomous/tie/invalid outcomes; SSH Max-Cut autonomous mean 0.596214; open-data autonomous mean 0.919064 | 6 packages; SSH Max-Cut delta +0.388205; open-data delta +0.014944 | Prospective package shape, machine-gradeable frontier steering, and selective evaluator-stress are now audited remotely, but short-budget average benchmark superiority is still not supported. |
| Open-data evaluator-stress pilot | co-pilot guardrailed selector mean balanced accuracy 0.934008 | autonomous accuracy-only selector 0.919064 | 5 SSH Ubuntu sklearn tasks, 8 candidates each; 1 selection change; aggregate delta +0.014944 | Gate triggering is selective: four clean tasks tie, while an imbalance stress task changes branch choice and improves balanced accuracy by 0.074720. |
...
The new open-data multi-task pilot sits between the toy micro-task and the full FML setting. It runs five scikit-learn tasks on the SSH Ubuntu host using the same candidate model portfolio and deterministic data splits for both conditions. The autonomous selector chooses branches by validation accuracy only. The IGRE evaluator-stress gate chooses by validation balanced accuracy with a macro-F1 guardrail. On four clean built-in tasks, both selectors choose the same branch and tie. On the deterministic imbalanced stress task, the gate changes selection from `svc_rbf_standardized` to `svc_rbf_balanced_standardized`, improving test balanced accuracy by 0.074720; aggregate mean balanced accuracy increases from 0.919064 to 0.934008, a delta of +0.014944. This does not prove broad co-pilot superiority. It supports a narrower design rule: evaluator-stress gates should be mostly inert on clean tasks and intervene when a primary metric creates a plausible validity risk. In repeated online paired FML smokes, valid Causality runs still produce 0 co-pilot benchmark wins, 1 autonomous win, and 1 tie; the Fairness run is a no-valid-branch failure case.

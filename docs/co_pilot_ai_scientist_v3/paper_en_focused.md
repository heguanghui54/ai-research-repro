...
| Live four-condition TFR replay | raw model judge labels E2ENet positive | strict preregistered rule relabels mixed | 2 live calls; short-term winner tie; frontier winner tie | Shows why TFR needs deterministic rule checking: model optimism is corrected when the short-term penalty condition is not met. |
| Cross-model TFR replay judge | Claude selects six-gate hybrid as frontier winner | strict label remains mixed; Gemini malformed JSON | 1 successful cross-model judge | Reduces same-model bias and identifies a concrete failure mode: gate actions did not propagate enough into experimental design. |
...
| TFR can test whether past reviews would have moved automated science toward later frontiers. | Supported as an operationalized replay protocol; candidate mining screens 120 comments, a small OpenAlex validation gives delayed candidates a +0.064 lexical frontier advantage over controls, and the first live four-condition replay has been executed. | Current validated probes find 0 strict delayed-value cases; the live replay is mixed after preregistered rule repair, so the long-horizon hypothesis remains unproven. |
...
We then execute the first preregistered four-condition replay on
`paper_105_review_1`, the E2ENet case. The generated conditions are paper-only,
raw-review-guided, six-gate-hybrid-guided, and shuffled-review-control. A
Monica-routed `gpt-4o-mini` judge initially labels the case positive, but the
deterministic preregistered rule relabels it `mixed_or_inconclusive`: the guided
artifacts do not have a lower short-term score than paper-only, so the
delayed-value condition is not met. A cross-model Claude judge independently
keeps the strict label mixed while selecting the six-gate artifact as the
frontier winner. Claude's rationale is diagnostically useful: gate actions name
deployment constraints, sparse representations, and prototype implementation,
but these actions do not propagate into named datasets, quantitative efficiency
metrics, or a mechanistic DSFF ablation. This is not delayed-value success. It
is the first executed evidence that TFR can catch model-judge optimism and
expose where gate routing fails to affect experiment design.

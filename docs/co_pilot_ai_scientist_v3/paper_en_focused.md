...
The same boundary applies to engineering adoption. A public IGRE skill that is easy to install, useful across topics, and adopted by researchers would be important systems evidence: it would show that the six-gate theory can become reusable research infrastructure rather than only a manuscript idea. However, repository stars, forks, or community uptake would measure usability and transfer, not scientific superiority. They should complement, not replace, blind expert review and matched benchmark evidence.

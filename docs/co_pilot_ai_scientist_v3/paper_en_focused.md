...
| Live four-condition TFR replay | raw model judge labels 3/3 cases positive; Claude frontier winner is six-gate in 3/3 cases | strict preregistered rule relabels 3/3 mixed; cross-model strict positives 0 | 3 cases; 6 live same-model calls; 3 cross-model Claude judges; strict positives 0 | Shows why TFR needs deterministic rule checking: model optimism is corrected when the short-term penalty condition is not met, while frontier-routing diagnostics can still be useful. |
...
| Prospective matched packages | 4 co-pilot or human-selected wins, including SSH Max-Cut mean normalized score 0.984419, open-data evaluator-stress mean 0.926426, and held-out trigger-policy mean 0.923263 | 3 autonomous/tie/invalid outcomes; SSH Max-Cut autonomous mean 0.596214; open-data autonomous means 0.924857 and 0.920681 | 7 packages; SSH Max-Cut delta +0.388205; open-data deltas +0.001569 and +0.002582 | Prospective package shape, machine-gradeable frontier steering, selective evaluator-stress, and held-out trigger-policy transfer are audited, but short-budget average benchmark superiority is still not supported. |
| Prospective attention/taste instrumentation | complete attention-cost gates 7/7; complete taste/insight gates 7/7 | no independent human-subject timing or live multi-researcher traces | 7 gate records; 21.00 total active review minutes; mean taste/insight score 3.447 | Operator-recorded cost evidence makes collaboration measurable, but it is not evidence that human gates improve paper quality or population-level efficiency. |
...
| Evaluator-stress trigger-policy transfer | held-out frozen trigger mean balanced accuracy 0.924320 | held-out autonomous 0.920681; held-out always-on gate 0.923263 | held-out frozen-trigger delta +0.003639; 2 wins, 0 losses, 23 ties; always-on has 2 held-out losses | `class_imbalance_trigger_0_94` is selected on the first split set, then frozen and evaluated on held-out split seeds, preserving imbalance benefit while avoiding clean-task overreach in this task family. |
...
We therefore analyze trigger policies over the same 25 paired selections, then run a stricter discovery-to-held-out transfer check. On the first split set, the always-on evaluator-stress policy has 2 wins, 2 losses, and 21 ties versus autonomous selection. A simple class-imbalance trigger, activated only when the validation majority-class baseline is at least 0.94, is selected by mean delta with loss and trigger-parsimony tie-breaks: it triggers on 5 dataset-split cases, obtains 2 wins, 0 losses, and 23 ties, and raises mean balanced accuracy to 0.927934 (delta +0.003077 versus autonomous). We then freeze this selected policy and evaluate it on 25 held-out split-seed selections. The frozen policy obtains mean balanced accuracy 0.924320, delta +0.003639 versus autonomous, 2 wins, 0 losses, and 23 ties; the held-out always-on gate obtains delta +0.002582 and still has 2 losses. This is split-heldout design evidence within one open-data task family, not a prospective causal result across new task families. Its methodological value is that it converts the mixed pilot into a concrete participation-mode rule: human/evaluator intervention should be gated by evaluator-risk signals rather than left permanently on.

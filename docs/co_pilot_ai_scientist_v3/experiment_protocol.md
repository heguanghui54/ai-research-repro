...
A first fresh online smoke trajectory is archived under
`experiments/online_full_gate_smoke_20260601_145720/`. It was generated by
`scripts/run_online_full_gate_smoke.py` and launched a new FML-bench Causality
frontier plus a one-iteration OpenEvolve knapsack run on `ubuntu-heshi`. It
exercises all five gate types in one run, but its one-step continuation worsens

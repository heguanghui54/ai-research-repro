...
generated by `scripts/summarize_online_paired_smokes.py`. Across the two
same-continuous online smokes with autonomous baselines, the benchmark
aggregate is `0` co-pilot wins, `1` autonomous win, and `1` tie; mean test MAE
is `0.754120` for co-pilot and `0.643337` for autonomous. Monica-routed model
reviewers prefer the co-pilot manuscripts in `4/4` reviewer calls, which is
recorded only as manuscript-measurement readiness, not independent expert
review.
...
setting, and the repeated summary now covers two paired runs. It remains a
tiny-budget benchmark-negative-or-tied result without independent paper-quality
review.

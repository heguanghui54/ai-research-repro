...
| Evaluator gates must reject non-executable branches before continuation. | `fml_fairness_gated_drafts_failed/`: two Fairness_fairlearn drafts failed validation due Fairlearn/sklearn API incompatibilities before producing a score. | Supported as failure-mode evidence | Present as a gate-design lesson, not as performance evidence. |

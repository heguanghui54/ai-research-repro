...
| The package is clean-clone reproducible. | Clean-clone audit: latest audited pushed branch reran candidate-frontier validation, rebuilt focused bilingual PDFs, validated TFR audit status, and found 612/612 manifest artifacts. | Supported for latest audited TFR validation path. | Keep current after each new pushed artifact by refreshing clean-clone audit. |

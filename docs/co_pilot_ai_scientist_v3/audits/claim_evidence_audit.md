...
| Selected-branch continuation can beat a same-task same-step autonomous baseline in one pilot. | Snapshot-seeded continuation test MAE `0.402170`; autonomous matched four-step baseline test MAE `0.421474`; live two-draft test MAE `0.640451`; earlier four-step smoke test MAE `0.617719`. | Partially supported | Keep caveat: one task/seed, snapshot seeding, compact budget schedule; do not generalize beyond pilot. |
...
- More matched autonomous vs human-gated comparisons across tasks and seeds; the first single-task matched run is archived but not sufficient.

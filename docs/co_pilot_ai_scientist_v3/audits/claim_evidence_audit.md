...
| The package is clean-clone reproducible. | Clean-clone audit: latest audited pushed branch reran LHTG/DVRS audit, candidate-frontier validation, focused bilingual PDF build, TFR audit, and package consistency, and found 618/618 manifest artifacts. | Supported for latest audited LHTG/DVRS validation path. | Keep current after each new pushed artifact by refreshing clean-clone audit. |

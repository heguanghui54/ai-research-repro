...
| The package is clean-clone reproducible. | Clean-clone audit: latest audited pushed branch reran candidate-frontier validation, rebuilt focused bilingual PDFs, validated TFR audit status, and found 615/615 manifest artifacts. | Supported for latest audited package-consistency validation path. | Keep current after each new pushed artifact by refreshing clean-clone audit. |

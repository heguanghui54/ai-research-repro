...
| The package is clean-clone reproducible. | Clean-clone audit: latest audited pushed branch at `aaa8127ca` reran LHTG/DVRS audit, candidate-frontier validation, focused bilingual PDF build, TFR audit, package consistency, and objective-delivery audit, and found 621/621 manifest artifacts. | Supported for latest audited objective-delivery validation path. | Keep current after each new pushed artifact by refreshing clean-clone audit. |

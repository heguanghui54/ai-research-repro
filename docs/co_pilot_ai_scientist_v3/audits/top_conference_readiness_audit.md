...
| AI Co-Scientist-style direction finding and hypothesis organization | Partially supported with live front-end smoke | Candidate files, literature matrix, idea gate, scientific-taste gate, and `experiments/hypothesis_frontier_smoke_20260602_021500/`, which uses two Monica-routed `gpt-4o-mini` calls to generate four candidate research frontiers, critique/rank them, and select `frontier_004` for possible next-budget evaluation. | Connect this hypothesis front end to a downstream prospective experiment/manuscript trajectory and compare against an autonomous front-end baseline. |

...
| Blind human expert review readiness | Prepared as evaluation packet, not completed evidence | `experiments/human_expert_blind_review_packet_20260602_143000/` contains reviewer index, reviewer instructions, six anonymized A/B pair files, a score-sheet template, a collection protocol, and a hidden condition key for the OpenReview equal-context pairs. `scripts/summarize_human_expert_blind_reviews.py` can aggregate completed score sheets and now reports condition wins, win rates, mean deltas with bootstrap CIs, and Fleiss' kappa when enough raters complete the same pairs. The template-summary smoke passes but correctly reports `not_enough_valid_votes`. | Recruit 3-5 qualified human reviewers, collect blinded scores, report inter-rater agreement, and only then upgrade this from readiness to evidence. |

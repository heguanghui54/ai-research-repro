...
| Blind human expert review readiness | Prepared as evaluation packet, not completed evidence | `experiments/human_expert_blind_review_packet_20260602_143000/` contains reviewer instructions, six anonymized A/B pair files, a score-sheet template, and a hidden condition key for the OpenReview equal-context pairs. `scripts/summarize_human_expert_blind_reviews.py` can aggregate completed score sheets. | Recruit 3-5 qualified human reviewers, collect blinded scores, report inter-rater agreement, and only then upgrade this from readiness to evidence. |

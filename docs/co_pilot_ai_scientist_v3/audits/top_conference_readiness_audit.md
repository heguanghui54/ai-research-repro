...
| AI Scientist-v2-style experiment execution and paper writing | Partially supported | FML-bench runs, matched Causality pairs, generated paper drafts and PDFs. A fresh online full-gate smoke now generates a complete co-pilot manuscript artifact with internal score `4.64`, plus a matched-budget autonomous manuscript comparator with internal score `3.48`. | Add a same-continuous-trajectory autonomous manuscript baseline and scale beyond smoke budgets. |
...
| Top-conference-level empirical support | Not yet supported | Monica-routed reviewers agree broader matched-budget evidence and same-continuous-trajectory manuscript comparisons are missing; Claude rejects for strong venue. | Treat current paper as a strong pilot/reproducibility package, not a completed top-conference submission. |

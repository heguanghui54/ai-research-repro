...
| Human participation at creative or key decision nodes | Supported as logging and orchestration | Human gate schema plus the derived Human Co-Pilot Trace Dataset: 29 gate records, 12 records with attention-cost fields, 7 records with taste/insight fields, and 3 prospective matched packages. The dataset release audit passes with 0 secret-pattern hits and 0 raw-log marker hits. | Expand beyond the single-author trace corpus and record future gates prospectively with both `taste_insight` and `attention_cost`. |
...
| Prospective matched-budget evidence package | Passes with mostly negative metric outcomes | `prospective_matched_budget_package_audit.md` now finds 3 passing packages, and `prospective_matched_package_summary.md` separates their outcomes: the controlled micro-pilot favors the human-selected branch (`0.984419` vs. `0.596214` mean normalized score), while two FML-bench Causality packages are negative for co-pilot performance (`0.646224` vs. `0.624703`, and `0.646224` vs. `0.296399` test MAE; lower is better). | Scale beyond small negative FML runs and evaluate high-tail paper-quality outcomes before upgrading claims. |

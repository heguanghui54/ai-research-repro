...
| AI Scientist-v2-style experiment execution and paper writing | Partially supported | FML-bench runs, matched Causality pairs, generated paper drafts and PDFs. Two same-continuous-trajectory online smokes now generate co-pilot and autonomous manuscript artifacts in the same orchestrator runs; the repeated summary reports `0` co-pilot benchmark wins, `1` autonomous win, and `1` tie. | Scale beyond smoke budgets across more tasks/seeds and add independent paper-quality review. |
...
| Top-conference-level empirical support | Not yet supported | The package now has two same-continuous-trajectory manuscript comparisons and model-review probes preferring co-pilot manuscripts, but the repeated benchmark aggregate is autonomous-or-tie and still tiny-budget. Broader matched evidence and independent human paper-quality review are still missing; Claude rejects for strong venue. | Treat current paper as a strong pilot/reproducibility package, not a completed top-conference submission. |

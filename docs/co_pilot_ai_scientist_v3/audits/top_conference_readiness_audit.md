...
| Human participation at creative or key decision nodes | Supported as logging and orchestration | Human gate schema plus the derived Human Co-Pilot Trace Dataset: 29 gate records, 12 records with attention-cost fields, 7 records with taste/insight fields, and 3 prospective matched packages. | Expand beyond the single-author trace corpus and record future gates prospectively with both `taste_insight` and `attention_cost`. |

...
| Evaluator-stress trigger-policy analysis | held-out 最佳触发策略 mean balanced accuracy 0.924320 | held-out autonomous 0.920681；held-out always-on gate 0.923263 | held-out trigger delta +0.003639；2 胜、0 负、23 平；触发 5 次 | 在第一组 split 中选出的 class-imbalance trigger 在 held-out split seeds 上仍然最佳，保留 imbalance 收益，同时避免干净任务 overreach。 |
...
因此，我们进一步在同一组 25 个 paired selections 上分析触发策略，并在另外 25 个 held-out split-seed selections 上重复验证。在第一组 split 中，always-on evaluator-stress policy 相对 autonomous 得到 2 胜、2 负、21 平。一个简单的 class-imbalance trigger 只在 validation majority-class baseline 至少为 0.94 时触发；它只触发 5 个 dataset-split cases，得到 2 胜、0 负、23 平，并把 mean balanced accuracy 提高到 0.927934（相对 autonomous delta +0.003077）。在 held-out split seeds 上，同一个 trigger 仍是最佳简约策略：mean balanced accuracy 为 0.924320，相对 autonomous delta +0.003639，2 胜、0 负、23 平；而 always-on gate 仍然有 2 个负例。这是在同一 open-data task family 内的 split-heldout 设计证据，不是跨新任务族的 prospective causal result。它的方法价值在于把混合 pilot 转化成具体的参与模式规则：人类/evaluator 介入应由 evaluator-risk signals 触发，而不是永远打开。

...
| 真实四条件 TFR 回放 | 原始模型 judge 将 3/3 case 标为 positive；Claude frontier winner 在 3/3 case 中都是 six-gate | 严格预注册规则将 3/3 修正为 mixed；cross-model strict positives 0 | 3 个 case；6 次 same-model 真实调用；3 次 Claude cross-model judge；strict positives 0 | 说明 TFR 需要确定性规则校验：当短期惩罚条件不成立时，模型乐观判断会被纠正，但 frontier-routing 诊断仍然有价值。 |
...
| Prospective matched packages | co-pilot 或人类选分支胜 4 次，其中 SSH Max-Cut mean normalized score 为 0.984419，open-data evaluator-stress mean 为 0.926426，held-out trigger-policy mean 为 0.923263 | autonomous / tie / invalid 3 次；SSH Max-Cut autonomous mean 为 0.596214，open-data autonomous mean 为 0.924857 和 0.920681 | 7 个 package；SSH Max-Cut delta +0.388205；open-data delta +0.001569 和 +0.002582 | prospective package 形状、机器可评分 frontier steering、选择性 evaluator-stress 和 held-out trigger-policy transfer 已被审计，但仍不支持短预算平均 benchmark 优越性。 |
| Prospective attention/taste instrumentation | attention-cost gates 完整 7/7；taste/insight gates 完整 7/7 | 没有独立 human-subject timing，也没有实时多研究者轨迹 | 7 条 gate records；总 active review minutes 21.00；mean taste/insight score 3.447 | operator-recorded 成本证据使协作可测量，但它不证明人类门控能提升论文质量或总体效率。 |
| Open-data evaluator-stress pilot | co-pilot guardrailed selector mean balanced accuracy 0.926426 | autonomous accuracy-only selector 0.924857 | 5 个 SSH Ubuntu sklearn task x 5 个 split seeds = 25 个 paired selections；4 次 selection change；aggregate delta +0.001569 | 多 seed 证据是混合的：co-pilot 2 胜、autonomous 2 胜、21 平；imbalance stress task 平均仍为正，但 breast cancer 暴露了一个小的 gate-overreach 负例。 |
| Evaluator-stress trigger-policy transfer | held-out 冻结触发策略 mean balanced accuracy 0.924320 | held-out autonomous 0.920681；held-out always-on gate 0.923263 | held-out frozen-trigger delta +0.003639；2 胜、0 负、23 平；always-on 有 2 个 held-out 负例 | `class_imbalance_trigger_0_94` 先在第一组 split 中被选出，再冻结到 held-out split seeds 上评估，在这个任务族中保留 imbalance 收益并避免干净任务 overreach。 |
...
| MLAgentBench CIFAR10/debug | co-pilot-selected branch 3 个 seed 的 official mean score 0.7743 | starter baseline official score 0.5103 | 一个官方 MLAgentBench task；mean delta +0.2640；min score 0.7709 | 更强的非 FML 官方证据，但仍只是一个官方任务，不是广泛 benchmark 覆盖。 |
| MLAgentBench OGBN-arxiv | co-pilot-selected branch 3 个 seed 的 official-evaluator mean score 0.5415 | compatibility starter score 0.02745 | mean delta +0.5141；min score 0.53999；sample std 0.001896 | 第二条 official-evaluator 路径，但 baseline 是兼容翻译，不是未改动的 NeighborLoader starter。 |
| Sklearn diabetes tabular probe | OpenEvolve median RMSE 55.895460 | direct rewrite RMSE 55.895460 | 无搜索优势 | 简单建模任务中直接编辑可能足够。 |

非 FML benchmark 扩展部分被有意同时报告“已评分 run”和“阻塞 setup 尝试”，而不是隐藏不利证据。除了已经可评分的 MLAgentBench vectorization 任务，我们把官方 MLAgentBench CIFAR10/debug 从慢速下载 blocker 推进为已评分 run：通过在 `ubuntu-heshi` 上预缓存官方 CIFAR10 archive，official prepare 成功完成；starter baseline 的官方 test accuracy 为 0.5103，而 co-pilot 选择的小型 batch-normalized CNN、轻量数据增强、AdamW、cosine decay、label smoothing、8 epoch 分支在首个 seed 下达到 0.7782。随后我们用相同 co-pilot-selected 分支追加两个 seed，官方分数分别为 0.7709 和 0.7738；三个 seed 的 mean score 为 0.7743，min score 为 0.7709，sample std 为 0.003676。OGBN-arxiv 现在也形成了三 seed 已评分 official-evaluator 路径：full-batch compatibility starter 在官方 `eval.py` 下得分 0.02745，co-pilot-selected normalized AdamW MLP 三个 seed 得分 0.53999、0.54089 和 0.54363，mean score 为 0.5415，min score 为 0.53999，sample std 为 0.001896，mean delta 为 +0.5141。但该 baseline 是为绕开当前环境缺失 `pyg-lib` / `torch-sparse` 而做的兼容翻译，不是未改动的 NeighborLoader starter。IMDB 仍被 Hugging Face 网络访问阻塞；CLRS 已到达 `train.py`，但 CPU 运行超时且没有生成 checkpoint；house-price 到达官方 prepare script，但需要 Kaggle tooling，并且很可能需要竞赛规则同意。因此，当前非 FML 官方证据边界已经增强，但仍不应被解释为广泛官方 benchmark 覆盖。
...
当前 prospective matched packages 被有意报告为混合结果，而不是包装成清晰的优越性证明。在已通过审计的包中，现在有两个受控 Max-Cut 案例显示 co-pilot 或人类选择分支有用，一个 open-data 多任务 evaluator-stress pilot 给出小幅正向 balanced-accuracy delta；但 3 个 FML-bench 案例中，co-pilot 分支出现失败、平局、中止或无法产生有效 continuation。SSH Ubuntu 上的 weighted Max-Cut 微型实验在同一组 12 个确定性的 8 节点 graph instances 上比较 autonomous alternating baseline 与 co-pilot 选择的 local-search branch，并用 brute-force optimum 计算 normalized score。autonomous baseline 的 mean normalized score 为 0.596214，minimum score 为 0.290323；co-pilot 选择分支的 mean normalized score 为 0.984419，minimum score 为 0.904762，mean delta 为 +0.388205。这是一个真实远程 matched-budget run，包含完整 gate logs 和 package audit，但它仍只是小型机器可评分子问题，不是 AI Scientist-v2 论文质量 benchmark。

新的 open-data 多任务 pilot 位于 toy micro-task 和完整 FML 之间。它在 SSH Ubuntu 主机上对 5 个 scikit-learn 任务运行 5 个 stratified split seeds，两种条件使用同一候选模型组合和匹配数据划分。autonomous selector 只根据 validation accuracy 选分支；IGRE evaluator-stress gate 则根据 validation balanced accuracy 并加入 macro-F1 guardrail 选分支。在 25 个 dataset-split paired selections 中，gate 改变分支选择 4 次，得到 co-pilot 2 胜、autonomous 2 胜、21 平。aggregate mean balanced accuracy 为 co-pilot 0.926426、autonomous 0.924857，delta 为 +0.001569。synthetic imbalance stress task 平均仍为正（delta +0.015385），但 breast cancer task 暴露出一个小的 gate-overreach 负例（delta -0.007540）。这不证明广义 co-pilot 优越性，但支持一个更窄的设计规则：evaluator-stress gate 应该大多数时候保持 inert，只在 primary metric 存在有效性风险时介入，并且需要触发策略来避免在干净任务上过度介入。在重复在线 paired FML smoke 中，有效 Causality 运行仍然得到 0 次 co-pilot benchmark 胜出、1 次 autonomous 胜出、1 次平局；Fairness 运行是 no-valid-branch 失败案例。

因此，我们进一步在同一组 25 个 paired selections 上分析触发策略，然后运行更严格的 discovery-to-held-out transfer check。在第一组 split 中，always-on evaluator-stress policy 相对 autonomous 得到 2 胜、2 负、21 平。一个简单的 class-imbalance trigger 只在 validation majority-class baseline 至少为 0.94 时触发；它按 mean delta、负例数和触发简约性 tie-break 被选中，触发 5 个 dataset-split cases，得到 2 胜、0 负、23 平，并把 mean balanced accuracy 提高到 0.927934（相对 autonomous delta +0.003077）。随后我们冻结这个已选策略，在另外 25 个 held-out split-seed selections 上评估。冻结策略 mean balanced accuracy 为 0.924320，相对 autonomous delta +0.003639，2 胜、0 负、23 平；held-out always-on gate 的 delta 为 +0.002582，且仍然有 2 个负例。这是在同一 open-data task family 内的 split-heldout 设计证据，不是跨新任务族的 prospective causal result。它的方法价值在于把混合 pilot 转化成具体的参与模式规则：人类/evaluator 介入应由 evaluator-risk signals 触发，而不是永远打开。

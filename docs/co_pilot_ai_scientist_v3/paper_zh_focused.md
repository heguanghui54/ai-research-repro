...
非 FML benchmark 扩展部分被有意同时报告“已评分 run”和“阻塞 setup 尝试”，而不是隐藏不利证据。除了已经可评分的 MLAgentBench vectorization 任务，我们把官方 MLAgentBench CIFAR10/debug 从慢速下载 blocker 推进为已评分 run：通过在 `ubuntu-heshi` 上预缓存官方 CIFAR10 archive，official prepare 成功完成；starter baseline 的官方 test accuracy 为 0.5103，而 co-pilot 选择的小型 batch-normalized CNN、轻量数据增强、AdamW、cosine decay、label smoothing、8 epoch 分支在首个 seed 下达到 0.7782。随后我们用相同 co-pilot-selected 分支追加两个 seed，官方分数分别为 0.7709 和 0.7738；三个 seed 的 mean score 为 0.7743，min score 为 0.7709，sample std 为 0.003676。IMDB 仍被 Hugging Face 网络访问阻塞。OGBN-arxiv 是更有希望的下一个官方候选：Stanford SNAP 数据可以下载，official prepare 在 PyTorch 兼容设置下可以完成，但 starter baseline 在生成 `submission.csv` 前失败，因为当前 torch 2.12 CPU 环境缺少 PyG neighbor sampling 所需的 `pyg-lib` 或 `torch-sparse`。CLRS 已到达 `train.py`，但 CPU 运行超时且没有生成 checkpoint；house-price 到达官方 prepare script，但需要 Kaggle tooling，并且很可能需要竞赛规则同意。因此，当前非 FML 官方证据边界已经增强，但仍只是一个官方任务；在评分另一个官方任务或扩展任务组合前，论文仍不应暗示已经具备广泛官方 benchmark 覆盖。

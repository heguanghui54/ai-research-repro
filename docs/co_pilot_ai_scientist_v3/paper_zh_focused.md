...
| Prospective matched packages | co-pilot 或人类选分支胜 3 次，其中 SSH Max-Cut mean normalized score 为 0.984419，open-data evaluator-stress mean 为 0.934008 | autonomous / tie / invalid 3 次；SSH Max-Cut autonomous mean 为 0.596214，open-data autonomous mean 为 0.919064 | 6 个 package；SSH Max-Cut delta +0.388205；open-data delta +0.014944 | prospective package 形状、机器可评分 frontier steering 和选择性 evaluator-stress 已能在远程机器上审计，但仍不支持短预算平均 benchmark 优越性。 |
| Open-data evaluator-stress pilot | co-pilot guardrailed selector mean balanced accuracy 0.934008 | autonomous accuracy-only selector 0.919064 | 5 个 SSH Ubuntu sklearn task，每个 8 个 candidate；1 次 selection change；aggregate delta +0.014944 | gate 触发是选择性的：4 个干净任务平局，在 imbalance stress task 上改变分支并使 balanced accuracy 提高 0.074720。 |
...
新的 open-data 多任务 pilot 位于 toy micro-task 和完整 FML 之间。它在 SSH Ubuntu 主机上运行 5 个 scikit-learn 任务，两种条件使用同一候选模型组合和确定性数据划分。autonomous selector 只根据 validation accuracy 选分支；IGRE evaluator-stress gate 则根据 validation balanced accuracy 并加入 macro-F1 guardrail 选分支。在 4 个干净内置任务上，两种 selector 选择相同分支并平局。在确定性的 imbalanced stress task 上，gate 把选择从 `svc_rbf_standardized` 改为 `svc_rbf_balanced_standardized`，test balanced accuracy 提高 0.074720；aggregate mean balanced accuracy 从 0.919064 提高到 0.934008，delta 为 +0.014944。这不证明广义 co-pilot 优越性，但支持一个更窄的设计规则：evaluator-stress gate 应在干净任务上基本保持 inert，而在 primary metric 存在有效性风险时介入。在重复在线 paired FML smoke 中，有效 Causality 运行仍然得到 0 次 co-pilot benchmark 胜出、1 次 autonomous 胜出、1 次平局；Fairness 运行是 no-valid-branch 失败案例。

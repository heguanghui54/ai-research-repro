...
| Evaluator-gate necessity | FML-bench `Fairness_fairlearn` plus controlled metric-gaming evaluator-stress smoke | Failed two-draft extension plus repair/degenerate-predictor analysis; `metric_gaming_evaluator_stress_smoke_20260602_171500` shows a primary-only fairness metric selects an all-negative predictor while a utility guardrail selects a useful candidate | Gates must reject non-executable branches and metric-gaming fairness solutions before continuation | A successful fairness-task improvement or full benchmark superiority |

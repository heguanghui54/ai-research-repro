...
- The Fairlearn compatibility shim helper was added for older installed
  Fairlearn APIs.
...
### 5. Official Fairlearn task also completed successfully
...
ssh ubuntu-heshi 'cd /home/heshi/work/FML-bench && \
  ...
  --model deepseek-chat \
  --provider DeepSeek \
  --output-dir /home/heshi/work/fmlbench-fairlearn-results-fixed3 \
  agent.ai_scientist_v2.max_steps=4'
```
...
This task required a small compatibility shim because the installed Fairlearn
version did not accept `random_state` in `ExponentiatedGradient.__init__`.
